# Docker Scenario 5 - Zombie Processes from Missing PID 1 Reaping

A multiprocessing Python app spawns child workers without reaping them. When the container's
main process (PID 1) isn't a real init system, finished children become zombies that are
never cleaned up. Diagnosed and fixed using --init / tini.
...
app.py
    ...
    def worker_task(worker_id):
        print(f"[Child PID: {os.getpid()}] Worker {worker_id} starting and exiting immediately.")
        # Process exits here. State transitions to ZOMBIE until parent reads exit status.

    def main():
        print(f"[Parent PID: {os.getpid()}] Starting parent process (running as PID 1 in container)...")
        for i in range(5):
            p = multiprocessing.Process(target=worker_task, args=(i,))
            p.start()
            # CRITICAL ERROR: p.join() is intentionally omitted!
        print(f"[Parent PID: {os.getpid()}] Spawned 5 workers without joining. Entering long sleep loop...")
        while True:
            time.sleep(10)
...
Dockerfile (multi-stage, non-root)
    ...
    CMD ["python", "script_that_spawns_zombies.py"]

## Build-Phase Failures (before the actual scenario could even run)

1. `--prefix=/install` cache-key error
   Cause: with no external dependencies in requirements.txt, pip never created the /install
   directory, so the later COPY --from=builder /install had nothing to copy.
   Fix: dropped --prefix=/install entirely; copied directly from Python's standard install
   path (site-packages + bin) instead.

2. USER placed before COPY
   Cause: switching to the non-root user before copying files caused permission errors during
   the copy phase.
   Fix: reordered so useradd and COPY happen before USER is set, matching the standard pattern
   (create user -> copy with --chown -> switch to that user).

3. Invalid ARG/ENV chaining
   Cause: tried to chain `ARG BUILD_ENV=prod \` directly into an ENV block with a trailing
   backslash. ARG and ENV are separate instructions and cannot be joined this way.
   Fix: split into two separate instructions.

4. Typo'd build flag and path
   `--no--cache` (double dash) instead of `--no-cache`, and `/usr/local/bi` instead of
   `/usr/local/bin`.
   Fix: corrected both by rereading the actual command before running it.
